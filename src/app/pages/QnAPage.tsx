...
export function QnAPage() {
  ...
  const questions = questionList.map((q: any) => ({
    id: q.id,
    title: q.title,
  ...
  const todayQuestions = questions.filter(q => q.date && q.date.includes(todayStr)).length;
  const activeUsers = new Set(questions.map(q => q.author).filter(Boolean)).size;

  const statItems = [
    { label: "전체 질문", value: totalQuestions.toLocaleString(), icon: MessageCircle },
    { label: "답변 완료", value: answeredQuestions.toLocaleString(), icon: Award },
    { label: "오늘의 질문", value: todayQuestions.toString(), icon: Calendar },
    { label: "활성 사용자", value: activeUsers.toString(), icon: User },
  ];
